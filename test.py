...
def extract_praat_features(wav_dir='C:\\AVEC2014\\audio\\dev', feature_dir='C:\\Features\\praat_features\\dev', project_directory='C:\\Darwin-Project', config_files=['cpp', 'ff', 'hnr'], tasks=['north', 'free']):
...
def build_feature_sets(csv_dir='C:\\Features\\praat_features', target_dir='', project_directory='C:\\Darwin-Project', tasks=['north', 'free']):
    data = {}
    if len(tasks) == 2:
        glob_path = f"{csv_dir}\\**\\*.csv"
    else:
        glob_path = f"{csv_dir}\\{tasks[0]}\\*.csv"

    print(glob_path)
    # build an organized dict with all data
    for file_path in glob.glob(glob_path, recursive=True):
        print(file_path)
        print(re.findall(r"(Development|Testing|Training|dev)/(Freeform|Northwind|free)", file_path))
        #[(partition, task)] = re.findall(r"(Development|Testing|Training|dev)/(Freeform|Northwind|free)", file_path)
