...
def extract_praat_features(wav_dir='C:\\AVEC2014\\audio\\dev', feature_dir='C:\\Features\\praat_opensmile_features\\dev',
                           project_directory='C:\\Darwin-Project', config_files=['cpp', 'ff', 'hnr'], tasks=['north', 'free']):
    ...
    for config_file in config_files:
        ...
        for file_path in glob.glob(glob_path, recursive=True):
            ...
            target_dir = os.path.join(feature_dir, file_dir)
            ...
            subprocess.run([f"{project_directory}\\data\\audio\\Praat\\praat_executable", '--run', config_path, file_path, target_file])

def extract_mfcc_features(wav_dir='C:\\AVEC2014\\audio\\dev', feature_dir='C:\\Features\\praat_opensmile_features\\dev',
                          project_directory='C:\\Darwin-Project', tasks=['north', 'free']):
    ...
    for file_path in glob.glob(glob_path, recursive=True):
        ...
        [_, file_dir] = os.path.split(file_dir)
        target_dir = os.path.join(feature_dir, file_dir)
        target_file = os.path.join(target_dir, f"{file_name[0: -4]}_mfcc.csv")
        if not os.path.exists(target_dir):
            os.makedirs(target_dir)

        # run shell praat command (windows version)
        subprocess.run(f"{open_smile_exe} -C {config_path} -I {file_path} -O {target_file}")

def build_feature_sets(csv_dir='C:\\Features\\praat_opensmile_features', target_dir='C:\\pickle_data', project_directory='C:\\Darwin-Project', tasks=['north', 'free']):
    data = {}
    if len(tasks) == 2:
        glob_path = f"{csv_dir}\\**\\*_cpp.csv"
    else:
        glob_path = f"{csv_dir}\\{tasks[0]}\\*_cpp.csv"

    # build an organized dict with all data
    for file_path in glob.glob(glob_path, recursive=True):
        file_path = file_path.split('_cpp.csv')[0]
        [(partition, task)] = re.findall(r"(Development|Testing|Training|dev)\\(Freeform|Northwind|free|north)", file_path)
        ...
        print(feature_number, cpp.shape, hnr.shape, formants.shape, delta_mfcc.shape)
    ...
    for p in data:
        f = open(os.path.join(target_dir, f"praat_opensmile_audio_features_{p.lower()}.pkl"), "wb")
