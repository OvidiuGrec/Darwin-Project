...
def scale(X_train, X_test, scale_type='minmax', axis=None, use_boxcox=False, boxcox_axis=None,
          use_pandas=False, verbose=False):
	"""
	...
	if use_boxcox:
		X_train, X_test = boxcox_transform(X_train, X_test, axis=boxcox_axis, verbose=verbose)
...
def boxcox_transform(X_train, X_test, axis=None, verbose=False):
	sv = 1  # Used to add before boxcox transformation to ensure all values are positive
	if axis == 0:
		for i in progressbar.progressbar(range(X_train.shape[1])):
